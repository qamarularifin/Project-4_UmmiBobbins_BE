...
router.post("/login", async (req, res) => {
  const user = await User.findOne({
    // user returns an object that is tied to the username i.e, 123
  ...
  const isPasswordValid = await bcrypt.compare(
    req.body.password,
    user.password
  ); //user.password is the object which has password as one of the keys
  ...
  if (isPasswordValid) {
    ...
    return res.json({ status: "ok", userData: req.session.user });
  } else {
    return res.json({ status: "error", userData: false });
  }
});
